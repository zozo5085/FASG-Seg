...
def preprocess(cfg, image, label, return_meta=False, unlabeled=True):
    ...
    edge_map = cv2.Canny(gray, 50, 150)  # 產生 uint8 邊緣圖

    edge_map = cv2.copyMakeBorder(
        edge_map,
        padding[1],  # top (0)
        padding[3],  # bottom (height)
        padding[0],  # left (0)
        padding[2],  # right (width)
        cv2.BORDER_CONSTANT,
        value=0  # 邊緣補黑 (無邊界)
    )

    edge_map = edge_map.astype(np.float32) / 255.0  # normalize to [0,1]
    edge_map = np.expand_dims(edge_map, axis=0)  # shape → (1,H,W)
    edge_map = torch.from_numpy(edge_map)  # → FloatTensor
...
def load_pseudo_label_lines(txt_path):
    """
    ReCLIP 系列 pseudo_label 檔格式：每一行像這樣：
    [1, 5, 12]
    []
    [3]
    """
...
def read_file_list(cfg):
    ...
    if dataset == 'context':
        ...
        # 先定義 pseudo 檔路徑，再讀 pseudo
        json_file_path = r'D:/ReCLIPv3/text/context_pseudo_label_ReCLIPPP.json'
        if os.path.exists(json_file_path):
            pseudo_classes = load_pseudo_label_lines(json_file_path)
        else:
            pseudo_classes = []

        # === 核心：用 pseudo 長度裁切 train list（不重做 pseudo 的情況下只能這樣）===
        if len(pseudo_classes) > 0 and len(pseudo_classes) != len(train_filenames):
            print(f"[WARN] context pseudo mismatch: train={len(train_filenames)} pseudo={len(pseudo_classes)}")
        ...
        pseudo_classes = []
        json_file_path = 'text/voc_pseudo_label_ReCLIPPP.json'
        try:
        ...
        # [恢復作者邏輯] 使用 os.listdir 但不排序，以配合舊 JSON

        # 為了保險，先確保路徑末端有斜線或正確 join
        ...
        # 讀 pseudo classes (指向你截圖中的檔案)
        pseudo_classes = []
        # 注意：如果你之前用 generate_ade20k_json.py 生成了新檔且有排序，這裡用舊檔會錯位
        # 但既然你要還原作者邏輯，我們就讀回舊檔
        json_file_path = r'D:/ReCLIPv3/text/ade_pseudo_label_ReCLIPPP.json'

        print(f"[INFO] Loading ADE20K Pseudo Labels from: {json_file_path}")
        if os.path.exists(json_file_path):
                ...
                for raw in file:
                    ...
                    if line == "":
                        ...
                        try:
                            ...
                            splitted_s = []
                    pseudo_classes.append(splitted_s)
            print(f"[INFO] Successfully loaded {len(pseudo_classes)} labels.")
        else:
            print(f"[WARNING] 找不到 {json_file_path}，使用空 pseudo_classes。")
            pseudo_classes = [[] for _ in range(len(train_images))]
        ...
        json_file_path = r'D:/ReCLIPv3/text/coco_pseudo_label_ReCLIPPP.json'

        if os.path.exists(json_file_path):

            pseudo_classes = load_pseudo_label_lines(json_file_path)  # 你前面那個 line-based parser

        else:

            pseudo_classes = [[] for _ in range(len(train_images))]

        # ✅ 嚴格檢查長度（不要默默裁切，會 shift）
        ...
        if len(pseudo_classes) != len(train_images):
            raise RuntimeError(
                ...
                f"Hint: 你的 train list 排序方式/資料集版本 必須和生成 pseudo label 的時候完全一致。"
        ...
        print(f"[INFO] 使用 Cityscapes 根目錄: {cfg.DATASET.DATAROOT}")
        ...
        pseudo_classes = []
        json_file_path = "D:/ReCLIPv3/text/cityscapes_pseudo_label_22972.json"

        print(f"[INFO] 讀取 pseudo labels: {json_file_path}")
        try:
